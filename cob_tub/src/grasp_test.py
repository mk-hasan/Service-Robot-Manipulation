...
class GraspScript(script):
	...
	def Run(self):
		...
		#self.sss.wait_for_input()
		handle_sdh = self.sss.move("gripper","cylopen",False)
		handle_sdh.wait()
		...
		object_pose_bl.pose.position.x = -0.82
		object_pose_bl.pose.position.y = -0.075
		object_pose_bl.pose.position.z = 1.0
		rospy.sleep(2)
		...
		if not self.sss.parse:
			...
			handle_arm.wait()
			handle01 = self.sss.move("gripper","cylclosed")
			handle01.wait()
